...
st_printArrPad(array, depth=5, indentLevel="", bracketpad := "0" , spacepad := " ")
{
	padc := Strlen(array.length())
	for k,v in Array
	{
		list.= indentLevel "[" fill(padc - Strlen(k),bracketpad)  k "]"
		if (IsObject(v) && depth>1)
			list.="`n" st_printArr(v, depth-1, indentLevel .  fill(4, spacepad) . fill(padc-1,spacepad))
		Else
			list.=" => " v
		list.="`n"
	}
...
fill(qty=1, sym=" ")
{
	if (qty<=0 || sym="")
		return ""
	out:=strReplace(format("{:0" qty "}", 0), 0, sym)
	return substr(out, 1, qty)
}
...
*/
;https://autohotkey.com/board/topic/93994-downloadtostring-timeout/
/*
countArr
...
RetryUdtv(url, maxRetries := 100, delayIncrement := 1) {
	Loop, % maxRetries {
		Try Return UrlDownloadToVar(url)
		Catch
			Sleep, % delayIncrement * A_Index * 1000
	}
	loop,5
		msgbox % "max retries reached for " clipboard := url 
}
...
;}
	...
	{ ;Decode xml required characters, as well as numeric character references
		...
		Else If RegExMatch(str,"&lt;|&amp;|&gt;|&quot;|&apos;",ent)	; other tokens
			StringReplace,str,str,% ent,% tokens[ent],all
		else
			return str
